...
function getCompletedChunks(file) {
    try {
...
function markChunkCompleted(file, idx) {
    try {
    ...
    return new Promise((resolve, reject) => {
        function tryDispatch() {
            if (uploadedCount === totalChunks) {
                if (ownWorkers) workers.forEach(w => w.terminate());
 ...
 * @param {function} [options.onError]
 */
export async function uploadDirectory(fileList, options = {}) {
    const {
        chunkSize = 1024 * 1024,
        maxConcurrentUploads = 3,
        resumable = true,
